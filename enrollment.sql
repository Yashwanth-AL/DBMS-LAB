...
SELECT c.cname, tb.book_title
FROM Course c
JOIN BookAdoption ba ON c.course = ba.course
JOIN TextBook tb ON tb.bookIsbn = ba.bookIsbn;

SELECT * FROM CourseRelatedBooks;

-- Create trigger to delete Enroll records when a course is deleted
DELIMITER //
...
DELETE FROM Course WHERE course = 2; -- This will delete related Enroll records
